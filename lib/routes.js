...
module.exports = function createRoutes(ctx) {
   ...
   * 而分镜表里本来就存了台词——这条链路是真的能接上的。
   * 没台词的镜头不占字幕位（空的字幕块导入后会变成一个空白条目）。
   */
  ...
  on('GET', '/api/edit-plans/assemble', (req, res, params, body, query) => {
    ...
    const clips = shots.map((s) => {
      const v = pickFor(s);
      const dur = num(s.duration_seconds, 5);
      return {
        storyboard_id: s.id,
        video_id: v ? v.id : '',
    ...
    return {
      ...
      episode,
      clips,
      stats: { total: clips.length, ready, missing: clips.length - ready },
    };
  });

  /** 导出交接清单：真实文件 + 顺序/入出点/转场 */
  on('GET', '/api/edit-plans/:id/export', (req, res, params) => {
    ...
    return {
      ...
      clips: timeline,
      transitions: trans,
      // SRT 字幕：OpenReel 可直接导入，台词来自分镜
      srt: buildSrt(timeline),
      srt_count: timeline.filter((c) => String(c.dialogue || '').trim()).length,
      ...
      openreel: {
        ...
        files: timeline.map((t) => ({ name: `${t.shot_number}_${t.name}`, source: t.file || t.url })),
