...
module.exports = function createRoutes(ctx) {
  ...
  on('POST', '/api/videos', async (req, res, params, body) => {
    ...
    // Agnes 抓不到 /assets/... 这种本机路径，提交前先把本地图传成公网地址
    ...
    try {
      ...
      const asset = store.insert('video_assets', {
        ...
        video_prompt: prompt,
        negative_prompt: str(body.negative_prompt),
        source_image_url: str(img.image || body.image),
        source_images: img.source_images || (Array.isArray(body.source_images) ? body.source_images : []),
