...
module.exports = function createRoutes(ctx) {
  ...
  function mediaReadable(asset) {
    ...
    if (local) {
      try { if (fs.existsSync(local) && fs.statSync(local).isFile() && fs.statSync(local).size > 0) return true; } catch { /* continue with URL */ }
    }
  ...
  function storyboardRow(r, idx) {
    return {
      ...
      linked_dialogue_audio_id: r.linked_dialogue_audio_id || null,
      linked_narration_audio_id: r.linked_narration_audio_id || null,
      production_revision: Math.max(1, num(r.production_revision, 1)),
      image_revision: Math.max(1, num(r.image_revision, num(r.production_revision, 1))),
      video_revision: Math.max(1, num(r.video_revision, num(r.production_revision, 1))),
      dialogue_revision: Math.max(1, num(r.dialogue_revision, num(r.production_revision, 1))),
      narration_revision: Math.max(1, num(r.narration_revision, num(r.production_revision, 1))),
  ...
  on('PUT', '/api/storyboards/:id', (req, res, params, body) => {
    const before = store.get('storyboards', params.id);
    if (!before) throw httpError(404, '分镜不存在');
    const patch = {};
    for (const k of ['episode_number', 'shot_number', 'shot_type', 'scene_description', 'characters',
      ...
      'image_prompt', 'video_prompt', 'negative_prompt', 'linked_image_id', 'linked_video_id', 'linked_audio_id', 'linked_dialogue_audio_id', 'linked_narration_audio_id',
      'status', 'sort_order']) {
      if (k in body) {
    ...
    const changed = (key) => key in patch && String(patch[key] ?? '') !== String(before[key] ?? '');
    const imageChanged = ['image_prompt', 'negative_prompt', 'scene_description', 'characters', 'scene', 'action'].some(changed);
    const videoChanged = imageChanged || ['video_prompt', 'duration_seconds'].some(changed);
    const dialogueChanged = changed('dialogue');
    const narrationChanged = changed('narration');
    const audioChanged = dialogueChanged || narrationChanged;
    if (imageChanged && !('linked_image_id' in body)) patch.linked_image_id = '';
    if (videoChanged && !('linked_video_id' in body)) patch.linked_video_id = '';
    if (audioChanged && !('linked_audio_id' in body)) patch.linked_audio_id = '';
    if (dialogueChanged && !('linked_dialogue_audio_id' in body)) patch.linked_dialogue_audio_id = '';
    if (narrationChanged && !('linked_narration_audio_id' in body)) patch.linked_narration_audio_id = '';
    if ((imageChanged || videoChanged || audioChanged) && !('status' in body)) {
      patch.status = imageChanged ? 'draft' : (videoChanged ? (before.linked_image_id ? 'image_ready' : 'draft') : (before.linked_video_id ? 'video_ready' : (before.linked_image_id ? 'image_ready' : 'draft')));
    }
    if (imageChanged || videoChanged || audioChanged) patch.production_revision = Math.max(1, num(before.production_revision, 1)) + 1;
    if (imageChanged) patch.image_revision = Math.max(1, num(before.image_revision, num(before.production_revision, 1))) + 1;
    if (videoChanged) patch.video_revision = Math.max(1, num(before.video_revision, num(before.production_revision, 1))) + 1;
    if (dialogueChanged) patch.dialogue_revision = Math.max(1, num(before.dialogue_revision, num(before.production_revision, 1))) + 1;
    if (narrationChanged) patch.narration_revision = Math.max(1, num(before.narration_revision, num(before.production_revision, 1))) + 1;
    return store.update('storyboards', params.id, patch);
  });
  ...
  on('POST', '/api/tts/generate', async (req, res, params, body) => {
    ...
    const audioType = str(body.audio_type || 'dialogue');
    const revisionField = audioType === 'narration' ? 'narration_revision' : 'dialogue_revision';
    const submittedRevision = production.storyboard ? Math.max(1, num(production.storyboard[revisionField], num(production.storyboard.production_revision, 1))) : null;
    ...
    const asset = store.insert('audio_assets', { project_id: production.project_id, storyboard_id: production.storyboard_id, storyboard_revision: submittedRevision, name: str(body.name) || `配音_${Date.now()}`, audio_type: str(body.audio_type || 'dialogue'), speaker: str(body.speaker), text, url: `/assets/audios/${name}`, local_file: name, duration: 0, status: 'completed', is_favorited: false, notes: '', provider: s.tts_provider, model_name: str(body.model || s.tts_model) });
    if (production.storyboard_id && ['dialogue', 'narration'].includes(asset.audio_type)) {
      ...
      const currentRevision = sb ? Math.max(1, num(sb[revisionField], num(sb.production_revision, 1))) : null;
      if (sb && Number(currentRevision) === Number(submittedRevision || 1)) {
        const audioLink = asset.audio_type === 'dialogue' ? 'linked_dialogue_audio_id' : 'linked_narration_audio_id';
        const patch = { linked_audio_id: asset.id, [audioLink]: asset.id };
        const updated = store.update('storyboards', production.storyboard_id, patch);
  ...
  on('POST', '/api/audio-assets', (req, res, params, body) => {
    ...
    const asset = store.insert('audio_assets', {
      ...
      storyboard_revision: body.storyboard_revision != null ? Math.max(1, num(body.storyboard_revision, 1)) : (production.storyboard ? Math.max(1, num(str(body.audio_type) === 'narration' ? production.storyboard.narration_revision : production.storyboard.dialogue_revision, num(production.storyboard.production_revision, 1))) : null),
    ...
    if (production.storyboard_id && ['dialogue', 'narration'].includes(asset.audio_type)) {
      const sb = store.get('storyboards', production.storyboard_id);
      const currentRevision = Math.max(1, num(asset.audio_type === 'narration' ? sb?.narration_revision : sb?.dialogue_revision, num(sb?.production_revision, 1)));
      if (sb && (asset.storyboard_revision == null || Number(asset.storyboard_revision) === Number(currentRevision))) {
        const audioLink = asset.audio_type === 'dialogue' ? 'linked_dialogue_audio_id' : 'linked_narration_audio_id';
        const updated = store.update('storyboards', sb.id, { linked_audio_id: asset.id, [audioLink]: asset.id });
        poller.events.emit('storyboard', updated || Object.assign({}, sb, { linked_audio_id: asset.id }));
      }
    }
    return asset;
  });
  ...
  function linkImageToStoryboard(asset) {
    ...
    if (!sb) return;
    if (asset.storyboard_revision != null && Number(asset.storyboard_revision) !== Number(sb.image_revision || sb.production_revision || 1)) return;
  ...
  on('POST', '/api/images', (req, res, params, body) => {
    ...
    const asset = store.insert('image_assets', {
      ...
      storyboard_revision: body.storyboard_revision != null ? Math.max(1, num(body.storyboard_revision, 1)) : (ctx.storyboard ? Math.max(1, num(ctx.storyboard.image_revision, num(ctx.storyboard.production_revision, 1))) : null),
  ...
  function videoAssetRecord(body, ctx, tail) {
    ...
    return Object.assign({
      ...
      storyboard_revision: production.storyboard ? Math.max(1, num(production.storyboard.video_revision, num(production.storyboard.production_revision, 1))) : null,
  ...
  function normalizeClips(raw) {
    ...
    return raw.slice(0, 500).map((c) => {
      ...
      return {
        ...
        audio_id: str(o.audio_id) || '',
        audio_url: str(o.audio_url),
        audio_local_file: str(o.audio_local_file),
        audio_type: str(o.audio_type),
        audio_duration: num(o.audio_duration, 0),
        audios: Array.isArray(o.audios) ? o.audios.slice(0, 8).map((a) => ({ id: str(a?.id), type: str(a?.type), url: str(a?.url), local_file: str(a?.local_file), duration: num(a?.duration, 0), speaker: str(a?.speaker) })) : [],
        dialogue_audio: o.dialogue_audio && typeof o.dialogue_audio === 'object' ? { id: str(o.dialogue_audio.id), url: str(o.dialogue_audio.url), local_file: str(o.dialogue_audio.local_file), duration: num(o.dialogue_audio.duration, 0) } : null,
        narration_audio: o.narration_audio && typeof o.narration_audio === 'object' ? { id: str(o.narration_audio.id), url: str(o.narration_audio.url), local_file: str(o.narration_audio.local_file), duration: num(o.narration_audio.duration, 0) } : null,
        audio_missing: !!o.audio_missing,
  ...
  /**
   * 把项目里填的「单集时长」文本解析成秒。
   * 用户填的是自由文本（默认「1分钟」），常见写法都得认：
   *   「1分钟」60 · 「90秒」90 · 「2分30秒」150 · 「1:30」90 · 「60」60
   * 认不出来返回 0 —— **不要编一个默认值冒充**，
   * 否则剪辑台会拿一个假目标去警告用户「时长不对」。
   */
  function parseDurationToSeconds(text) {
    const s = String(text == null ? '' : text).trim();
    if (!s) return 0;

    // 纯数字：按秒算（用户填 60 通常就是想要 60 秒）
    if (/^\d+(\.\d+)?$/.test(s)) return Math.round(Number(s));

    // mm:ss 或 hh:mm:ss
    const colon = s.match(/^(?:(\d+):)?(\d{1,2}):(\d{1,2})$/);
    if (colon) {
      const h = Number(colon[1] || 0), m = Number(colon[2]), sec = Number(colon[3]);
      return h * 3600 + m * 60 + sec;
    }

    // 中文：X分Y秒 / X分钟 / Y秒（分和秒都可有）
    const cn = s.match(/^(?:(\d+(?:\.\d+)?)\s*(?:分钟|分))?\s*(?:(\d+(?:\.\d+)?)\s*秒)?$/);
    if (cn && (cn[1] || cn[2])) {
      return Math.round(Number(cn[1] || 0) * 60 + Number(cn[2] || 0));
    }
    return 0;
  }
  ...
  on('GET', '/api/edit-plans/assemble', (req, res, params, body, query) => {
    ...
    const videos = store.list('video_assets', {
      filter: (r) => r.project_id === projectId && r.status === 'completed' && mediaReadable(r),
    ...
    const pickFor = (shot) => {
      // linked_video_id 是当前版本的权威引用。为空表示生产输入已修改、旧视频已经失效；
      // 不能再按 storyboard_id 从历史素材中把旧版本捞回来。
      if (!shot.linked_video_id) return null;
      return videos.find((v) => v.id === shot.linked_video_id) || null;
    };
    ...
    const clips = shots.map((s) => {
      ...
      const legacyAudio = s.linked_audio_id ? audios.find((a) => a.id === s.linked_audio_id) : null;
      const dialogueAudio = s.linked_dialogue_audio_id ? audios.find((a) => a.id === s.linked_dialogue_audio_id) || null : (legacyAudio?.audio_type === 'dialogue' ? legacyAudio : null);
      const narrationAudio = s.linked_narration_audio_id ? audios.find((a) => a.id === s.linked_narration_audio_id) || null : (legacyAudio?.audio_type === 'narration' ? legacyAudio : null);
      const shotAudios = [dialogueAudio, narrationAudio].filter(Boolean);
      const audio = dialogueAudio || narrationAudio;
      return {
        storyboard_id: s.id,
        video_id: v ? v.id : '',
    ...
    // 项目里填的「单集时长」——剪辑台要拿它检查成片长度对不对得上平台要求。
    // 解析不出来就是 0，前端据此不显示对比，而不是拿个默认值糊弄。
    const proj = store.get('projects', projectId);
    const targetLabel = str(proj && proj.episode_duration);
    return {
      ok: true,
      episode,
      clips,
      ...
      target_seconds: parseDurationToSeconds(targetLabel),
  ...
  on('GET', '/api/edit-plans/:id/export', (req, res, params, body, query) => {
    ...
    // 已保存方案保留人工排序/裁切，但媒体引用必须在导出时重新对齐当前分镜。
    // 否则旧视频/旧配音被删除或重新生成后，方案里缓存的旧 URL 仍会被当成有效素材导出。
    const projectVideos = store.list('video_assets', {
      filter: (v) => v.project_id === p.project_id && v.status === 'completed' && mediaReadable(v),
      sort: (a, b) => String(b.completed_at || b.updated_at || b.created_at || '').localeCompare(String(a.completed_at || a.updated_at || a.created_at || '')),
    });
    const projectAudios = store.list('audio_assets', {
      filter: (a) => a.project_id === p.project_id && a.status === 'completed' && mediaReadable(a),
      sort: (a, b) => String(b.updated_at || b.created_at || '').localeCompare(String(a.updated_at || a.created_at || '')),
    });
    const refreshedClips = (p.clips || []).flatMap((c) => {
      if (!c.storyboard_id) return [c];
      const sb = store.get('storyboards', c.storyboard_id);
      if (!sb) return []; // 已删除分镜不进入最终导出，也不作为可恢复的缺失素材保留
      // 只认 storyboard 当前明确关联的视频；关联被清空代表旧版本已经因内容修改而失效。
      const video = sb.linked_video_id ? projectVideos.find((v) => v.id === sb.linked_video_id) || null : null;
      const legacyAudio = sb.linked_audio_id ? projectAudios.find((a) => a.id === sb.linked_audio_id) : null;
      const dialogueAudio = sb.linked_dialogue_audio_id ? projectAudios.find((a) => a.id === sb.linked_dialogue_audio_id) || null : (legacyAudio?.audio_type === 'dialogue' ? legacyAudio : null);
      const narrationAudio = sb.linked_narration_audio_id ? projectAudios.find((a) => a.id === sb.linked_narration_audio_id) || null : (legacyAudio?.audio_type === 'narration' ? legacyAudio : null);
      const shotAudios = [dialogueAudio, narrationAudio].filter(Boolean);
      const duration = video ? (realDuration(video) || num(c.duration, num(sb.duration_seconds, 5))) : num(c.duration, num(sb.duration_seconds, 5));
      const trimIn = Math.min(Math.max(0, num(c.trim_in, 0)), Math.max(0, duration - 0.1));
      const trimOut = Math.max(trimIn + 0.1, Math.min(num(c.trim_out, duration), duration));
      return [{
        ...c,
        video_id: video ? video.id : '', url: video ? str(video.video_url) : '', local_file: video ? str(video.local_file) : '',
        duration, trim_in: trimIn, trim_out: trimOut, missing: !video,
        dialogue_audio: dialogueAudio ? { id: dialogueAudio.id, url: str(dialogueAudio.url), local_file: str(dialogueAudio.local_file), duration: num(dialogueAudio.duration, 0) } : null,
        narration_audio: narrationAudio ? { id: narrationAudio.id, url: str(narrationAudio.url), local_file: str(narrationAudio.local_file), duration: num(narrationAudio.duration, 0) } : null,
        audios: shotAudios.map((a) => ({ id: a.id, type: str(a.audio_type), url: str(a.url), local_file: str(a.local_file), duration: num(a.duration, 0), speaker: str(a.speaker) })),
      }];
    });
    const clips = refreshedClips.filter((c) => c.enabled !== false && !c.missing);
    ...
    const timeline = clips.map((c, i) => {
      ...
      const item = {
        index: i,
        storyboard_id: str(c.storyboard_id),
    ...
    const missingMaterials = refreshedClips.filter((c) => c.enabled !== false).flatMap((c) => {
      const out = [];
    ...
    return {
      ok: true,
      plan: { id: p.id, name: str(p.name), episode: num(p.episode_number, 1), clips: refreshedClips.map((c) => ({ ...c, audio_id: '', audio_url: '', audio_local_file: '', audio_type: '', audio_duration: 0 })), transitions: p.transitions || [] },
      settings: p.settings || { width: 1152, height: 768, frameRate: 24 },
      total_duration: Number(cursor.toFixed(3)),
      // 目标时长来自项目设置；解析不出来就是 0（前端不显示对比）
      target_seconds: (() => {
        const proj = store.get('projects', p.project_id);
        return parseDurationToSeconds(str(proj && proj.episode_duration));
      })(),
      clips: timeline,
      transitions: trans,
  ...
  on('POST', '/api/agnes/image', async (req, res, params, body) => {
    ...
    try {
      ...
      const asset = store.insert('image_assets', {
        ...
        storyboard_id: production.storyboard_id,
        name: str(body.name) || `图片_${Date.now()}`,
        storyboard_revision: production.storyboard ? Math.max(1, num(production.storyboard.image_revision, num(production.storyboard.production_revision, 1))) : null,
  ...
  return {
    dispatch, httpError, saveImageBase64, fetchRemoteImage, safeName, refreshModels,
