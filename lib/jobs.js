...
function persist() {
  ...
  try {
    ...
    fs.writeFileSync(tmp, JSON.stringify(Array.from(jobs.values()).slice(-50), null, 2), 'utf8');
    fs.renameSync(tmp, stateFile);
  } catch { /* 队列状态落盘失败不能阻断实际生成 */ }
}

function init(home) {
  stateFile = path.join(home, 'batch-jobs.json');
  jobs.clear();
  try {
    const saved = JSON.parse(fs.readFileSync(stateFile, 'utf8'));
    if (Array.isArray(saved)) saved.slice(-50).forEach((job) => {
...
function create(type, total) {
  ...
  const job = {
    ...
    concurrency: null,   // run() 里填入，让「并发到底用了多少」可被观测/断言
...
async function run(job, items, worker, opts = {}) {
  ...
  // 提交间隔：对着 Agnes 这种按次计费的上游，连着发容易被限流/判重。
  // 视频批量默认带一点间隔，图片批量不需要。
  ...
  async function loop() {
    while (cursor < items.length) {
      ...
      persist();
      onProgress(job);
      if (intervalMs > 0 && cursor < items.length && !job.cancel) {
...
module.exports = { init, create, get, list, run, cancel, prune };
